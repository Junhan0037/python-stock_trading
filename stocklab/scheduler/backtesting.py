...
def run_trading_scenario(code_list, date):
    ...
    while tick < 20: # 9시 ~ 9시 20분까지만 테스트
        print("ticK:", tick)
        for code in code_list:
            current_price = ebest_ace.get_price_n_min_by_code(date, code, tick) # 과거 가격 정보를 조회
...
if __name__ == '__main__':
    ...
    codes = ["180640", "005930"]
    day = datetime.now() - timedelta(days=4) # 4일전 데이터를 기반으로 백테스팅
