...
class XASession: # 서버 연결, 로그인
    ...
    def OnLogin(self, code, msg): # 로그인되면 발생하는 이벤트 (code가 0000이면 로그인 성공)
        if code == "0000":
            print(code, msg)
    ...
    def OnDisconnect(self): # 서버와 연결이 끊이지면 발생하는 이벤트
...
class XAQuery: # 조회 TR
    ...
    def OnReceiveData(self, code): # 서버로부터 데이터를 수신했을 때 발생
        print("OnReceiveData", code)
        XAQuery.tr_run_state = 1

    def OnReceiveMessage(self, error, code, message): # 서버로부터 메시지를 수신했을 때 발생
...
class EBest:
    ...
    LIMIT_SECONDS = 600 # 10min, 요청을 제한하여 block상태가 되는걸 방지
    ...
    def _execute_query(self, res, in_block_name, out_block_name, *out_fields, **set_fields): # *위치인자, **키워드 인자
        """
        ...
        while len(self.query_cnt) >= EBest.QUERY_LIMIT_10MIN: # block 상태를 방지하기 위해 요청을 10분에 200회로 제한
            time.sleep(1)
        ...
        for key, value in set_fields.items():
            xa_query.SetFieldData(in_block_name, key, 0, value) # TR 실행 전, 필요한 값을 채운다
        errorCode = xa_query.Request(0) # TR 요청

        # 요청 후 대기
        waiting_cnt = 0
        while xa_query.tr_run_state == 0: # 서버로부터 데이터를 수신하면 1로 바뀐다
            waiting_cnt += 1
            if waiting_cnt % 1000000 == 0:
                print("Waiting....", self.xa_session_client.GetLastError()) # waiting 메시지 조금씩 출력
            pythoncom.PumpWaitingMessages()
    ...
    def get_code_list(self, market=None): # 주식 종목 조회 메서드
        """
    ...
    def get_stock_price_by_code(self, code=None, cnt="1"): # 기간별 주가 메서드
        """
        ...
        result = self._execute_query("t1305",
                                     ...
                                     *out_params,
                                     **in_params)
        for item in result: # 나중에 DB에 요청한 코드가 필요하기 때문에 code 항목을 추가한다
            item["code"] = code
    ...
    def get_credit_trend_by_code(self, code=None, date=None): # 신용거래 동향 메서드
        """
        TR: t1921 신용거래동향
        :param code:str 종목코드
    ...
    def get_agent_trend_by_code(self, code=None ,fromdt=None, todt=None): # 외인 기관별 종목별 동향 메서드
    ...
    def get_short_trend_by_code(self, code=None, sdate=None, edate=None): # 공매도 추이 메서드
        """TR: t1927 공매도일별추이
        :param code:str 종목코드
    ...
    def get_account_info(self): # 계좌 금액을 조회하는 API
        """
    ...
    def get_account_stock_info(self): # 계좌의 보유 주식 종목을 조회하는 API
        """
    ...
    def order_stock(self, code, qty, price, bns_type, order_type="00"): # 현물 정상 주문 API
        """
        TR: CSPAT00600 현물 정상 주문
        :param bns_type:str 매매타입, 1:매도, 2:매수
        :param order_type:str 호가유형, 00:지정가, 03:시장가, 05:조건부지정가, 07:최우선지정가 61:장개시전시간외 종가, 81:시간외종가, 82:시간외단일가
    ...
    def order_cancel(self, order_no, code, qty): # 현물 취소 주문 API
        """
    ...
    def order_check(self, order_no=None): # 주식 체결 / 미체결 API
        """
        TR: t0425 주식 체결 / 미체결
        :param code:str 종목코드
        :param order_no:str 주문번호
        ...
        out_params = ["ordno", "expcode", "medosu", "qty", "price", "cheqty", "cheprice", "ordrem", "cfmqty", "status",
                      "orgordno", "ordgb", "ordermtd", "sysprocseq", "hogagb", "price1", "orggb", "singb", "loandt"]
    ...
    def get_current_call_price_by_code(self, code=None): # 주식 현재가 호가 조회 API
        """
    ...
    def get_tick_size(self, price): # 주식 가격 단위로 호가 단위를 구하는 메서드
        """
        호가 단위 조회
        참고:    
    ...
    def get_price_n_min_by_code(self, date, code, tick=None): # 주식차트(N분)의 메서드
        """
        TR: t8412 주식차트(N분) 
        :param code:str 종목코드
        :param date:str 시작시간
